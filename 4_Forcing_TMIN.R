...
lati = read.table("D:\\data_xijiang\\data_result\\forcing\\TMIN\\SURF_CLI_CHN_MUL_DAY-PRE-13011-201001.TXT")[,2]
lati = lati/100
long = read.table("D:\\data_xijiang\\data_result\\forcing\\TMIN\\SURF_CLI_CHN_MUL_DAY-PRE-13011-201001.TXT")[,3]
long = long/100
S_code = read.table("D:\\data_xijiang\\data_result\\forcing\\TMIN\\SURF_CLI_CHN_MUL_DAY-PRE-13011-201001.TXT")[,1]
#这里提取的是站点编码
fdata<-data.frame(lati,long,S_code)
write.table(fdata,"D:\\data_xijiang\\data_result\\forcing\\arcgis_input_TMIN_long_lati.txt",row.names=F,col.names=F,quote=F)
...
v3=length(S_code)
#v3是研究流域所需站点个数
#以上所有代码的作用是输出研究流域所有站点的代码，其他气象要素也一样适用 
#上次做到这里


path <- "D:\\data_xijiang\\data_result\\forcing\\tem"
#读取文件夹路径
FileNames <- dir(path)
#获取文件名
...
V_TMIN <- read.table(file=FilePath[i])[,10]
v2=length(V_TMIN)
...
for(j in 1:v2){
  if(V_TMIN[j]==32766){
    ...
    if(j>5){
      V_sum=0
      V_sum=V_TMIN[j-5]+V_TMIN[j-4]+V_TMIN[j-3]+V_TMIN[j-2]+V_TMIN[j-1]
      V_TMIN[j]=V_sum/5
      V_TMIN[j]=round(V_TMIN[j])
    }
  }
}
#这里可能也会出现32766，这里排32766思想是用前五天的数据相加求均值来代替这一天的数据
#排32700
f_data=data.frame(V_S,V_lati,V_long,V_year,V_month,V_day,V_TMIN)
...
write.table(m_data,file=paste("D:\\data_xijiang\\data_result\\forcing\\XJ_forcing_data\\TMIN\\XJ_TMIN",V_year[1],V_month[1],".txt",sep="_"),row.names=F,col.names=F,quote=F)
}
