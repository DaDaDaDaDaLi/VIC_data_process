...
lati = read.table("D:\\data_xijiang\\data_result\\forcing\\TMAX\\SURF_CLI_CHN_MUL_DAY-PRE-13011-201001.TXT")[,2]
lati = lati/100
long = read.table("D:\\data_xijiang\\data_result\\forcing\\TMAX\\SURF_CLI_CHN_MUL_DAY-PRE-13011-201001.TXT")[,3]
long = long/100
S_code = read.table("D:\\data_xijiang\\data_result\\forcing\\TMAX\\SURF_CLI_CHN_MUL_DAY-PRE-13011-201001.TXT")[,1]
#这里提取的是站点编码
fdata<-data.frame(lati,long,S_code)
write.table(fdata,"D:\\data_xijiang\\data_result\\forcing\\arcgis_input_TMAX_long_lati.txt",row.names=F,col.names=F,quote=F)
...
V_TMAX <- read.table(file=FilePath[i])[,9]
v2=length(V_TMAX)
...
for(j in 1:v2){
  if(V_TMAX[j]==32766){
    ...
    if(j>5){
      V_sum=0
      V_sum=V_TMAX[j-5]+V_TMAX[j-4]+V_TMAX[j-3]+V_TMAX[j-2]+V_TMAX[j-1]
      V_TMAX[j]=V_sum/5
      V_TMAX[j]=round(V_TMAX[j])
    }
  }
}
#这里可能也会出现32766，这里排32766思想是用前五天的数据相加求均值来代替这一天的数据
#排32700
f_data=data.frame(V_S,V_lati,V_long,V_year,V_month,V_day,V_TMAX)
...
write.table(m_data,file=paste("D:\\data_xijiang\\data_result\\forcing\\XJ_forcing_data\\TMAX\\XJ_TMAX",V_year[1],V_month[1],".txt",sep="_"),row.names=F,col.names=F,quote=F)
}
