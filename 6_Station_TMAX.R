...
Path="D:\\data_xijiang\\data_result\\forcing\\XJ_forcing_data\\TMAX"
Filenames<-dir(Path)
...
for(i in 1:v1){
  ...
  df<-data.frame(V_S_code,lati,long,year,month,day,TMAX)
  m_df<-as.matrix(df)
  v2<-length(TMAX)
  ...
  if(month[1] %in% c(1,3,5,7,8,10,12)){
    ...
    for(k in 1:31){
      write.table(data[,,k],paste("D:\\data_xijiang\\data_result\\forcing\\XJ_day_forcing_data\\TMAX\\day_TMAX",year[1],month[1],k,".txt",sep = "_"),col.names = F,row.names = F,quote = F)
    }
  }
