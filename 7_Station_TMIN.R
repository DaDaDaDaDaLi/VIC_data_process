...
Path="D:\\data_xijiang\\data_result\\forcing\\XJ_forcing_data\\TMIN"
Filenames<-dir(Path)
...
for(i in 1:v1){
  ...
  df<-data.frame(V_S_code,lati,long,year,month,day,TMIN)
  m_df<-as.matrix(df)
  v2<-length(TMIN)
  
  
  if(month[1] %in% c(1,3,5,7,8,10,12)){
    data<-array(,c(104,7,31))
    r=1
    k=1
    for(j in 1:v2){
    ...
    for(k in 1:31){
      write.table(data[,,k],paste("D:\\data_xijiang\\data_result\\forcing\\XJ_day_forcing_data\\TMIN\\day_TMIN",year[1],month[1],k,".txt",sep = "_"),col.names = F,row.names = F,quote = F)
    }
  }
  else if(month[1] %in% c(4,6,9,11)){
    data<-array(,c(104,7,30))
    r=1
    k=1
    for(j in 1:v2){
      C_S_code=df$V_S_code[j]
