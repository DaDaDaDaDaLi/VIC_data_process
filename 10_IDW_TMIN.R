Path="D:\\data_xijiang\\data_result\\forcing\\XJ_day_forcing_data\\TMIN"
Filenames<-dir(Path)
...
sample_point<-read.table("D:\\data_xijiang\\data_result\\forcing\\XJ_day_forcing_data\\TMIN\\day_TMIN_2010_1_1_.txt")[,c(2,3,7)]
...
for(i in 1:n1){
  ...
  TMIN<-read.table(file=FilePath[i])[,7]
  df<-data.frame(V_S_code,lati,long,year,TMIN)
  ...
  write.table(V_grid,file=paste("D:\\data_xijiang\\data_result\\forcing\\XJ_IDW\\TMIN\\TMIN",year[1],month[1],day[1],".txt",sep = "_"),col.names = F,row.names=F,quote = F)
}
...
if(F){
  ...
  Filenames<-read.table("C:\\Users\\Administrator\\Desktop\\vic\\result\\forcing\\grid_TMIN_filename.txt")
